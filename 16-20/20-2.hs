...
go types outputs = foldl lcm 1 $ getCycle <$> cycleList
    where
        ...
        process (mm@(Message s r m):ms) state = case Map.lookup r types of
            ...
            Just Conjunction -> (mm:finalCount, finalState)
                where
                    newState = conjState mm state
                    (msgCount, newMsgs) = send (conjCheck r newState) r
                    (finalCount, finalState) = process (ms ++ newMsgs) newState
            Just Broadcaster -> (mm:finalCount, finalState)
                where
                    (msgCount, newMsgs) = send m r
                    (finalCount, finalState) = process (ms ++ newMsgs) state
            _ -> process ms state

        button (_, state) = process [Message "button" "broadcaster" False] state

        checkMsgs a = any (\(Message _ r m) -> a == r && not m)

        getCycle a = fst $ head $ dropWhile (not . checkMsgs a . fst . snd) $ zip [0..] (iterate button ([], initState))
