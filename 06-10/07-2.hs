...
replaceJokersWith :: [Char] -> String -> [String]
replaceJokersWith cs s
    | [] <- s = [""]
    | 'J':ss <- s = concatMap ((<*>) ((:) <$> cs) . pure) $ replaceJokersWith cs ss
    | x:ss <- s = (x:) <$> replaceJokersWith cs ss

replaceJokers :: String -> [String]
replaceJokers s = replaceJokersWith (nub s) s
...
bestScore s = maximum $ score . makeHand <$> replaceJokers s
